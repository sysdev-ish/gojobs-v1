      ...
      <div class="careerfy-joblisting-classic-wrap" style="box-shadow: none; border-color: #d1d1d1;">
        ...
        <div class="careerfy-joblisting-text">
          <div class="careerfy-list-option">
            <h2 style="font-size: 18px; margin-bottom: 8px;"><a href="#"><?php echo Html::a($jobfunction, ['#', 'id' => $model->id], ['class' => 'btn-link', 'data-toggle' => 'tooltip', 'title' => 'View detail']); ?></a> <span><?php $date = date_create($model->lup); echo date_format($date, "d-M-Y"); ?></span>
            </h2>
            <ul>
              <li><i class="careerfy-icon careerfy-user"></i><?php echo $model->gender; ?></li>
              <li><i class="careerfy-icon careerfy-maps-and-flags"></i> <?php echo ($model->city) ? $model->city->city_name : ''; ?> / <?php if ($model->transjo->n_project == "Pilih") {
                echo "";
              } else {
                echo $model->transjo->n_project;
              } ?></li>
            ...
            <p>Job Requirement: <?php if ($model->transjo->syarat == "Terlampir" or $model->transjo->syarat =="TERLAMPIR" or $model->transjo->syarat == "SESUAI LAMPIRAN") {
              echo "-";
            } else {
              echo $model->transjo->syarat;
            } ?>
            </p>
            <p>Job Description: <?php if ($model->transjo->deskripsi == "Terlampir" or $model->transjo->deskripsi == "TERLAMPIR" or $model->transjo->deskripsi == "SESUAI LAMPIRAN") {
              echo "-";
            } else {
              echo $model->transjo->deskripsi;
            } ?>
            </p>
          </div>
