...
if(Yii::$app->utils->permission($role,'m45')){
  $action = '{tproc}';
}else{
  $action = ' ';
}
?>
<div class="taktif-index box box-solid">
                  ...
                  'tproc' => function($url,$model,$key){
                    ($model->status == 1 )?$disabled = false : $disabled = true;
                        $btn = Html::button('<i class="fa fa-tags" style="font-size:12pt;"></i>',[
                            'value'=>Yii::$app->urlManager->createUrl('taktif/tproc?id='.$model->id), //<---- here is where you define the action that handles the ajax request
