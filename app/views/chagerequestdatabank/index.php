...
$this->title = 'Change Request Data Bank Account';
...
$actiondelete = '';
$actionapprove = '';
if(Yii::$app->utils->permission($role,'m57')){
  $actionview = '{view}';
}
if(Yii::$app->utils->permission($role,'m59')){
  $actionupdate = '{update}';
}
if(Yii::$app->utils->permission($role,'m60')){
  $actiondelete = '{delete}';
}
if(Yii::$app->utils->permission($role,'m61')){
                ...
                'buttons'=>[
                  'view' => function($url,$model,$key){
                      $btn = Html::button('<i class="fa fa-eye" style="font-size:12pt;"></i>',[
                          'value'=>Yii::$app->urlManager->createUrl('chagerequestdatabank/view?id='.$model->id),
                  ...
                  'approve' => function($url,$model,$key){
                    if($model->status ==  2 AND Yii::$app->user->identity->id == $model->approvedby){
                      $disabled = false;
                      $url = Yii::$app->urlManager->createUrl('chagerequestdatabank/approve?id='.$model->id);
                    }else if ($model->status ==  3 AND Yii::$app->user->identity->id == $model->approvedby2){
                      $disabled = false;
                  ...
                  'update' => function ($url, $model) {
                    if($model->status < 2 OR $model->status == 6){
