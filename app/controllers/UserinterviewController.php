  ...
  public function actionUintproc($id)
  {
    ...
    if ($model->load(Yii::$app->request->post())) {
      ...
      if ($model->save(false) && $modelreccan->save()) {
        ...
        $body = Yii::$app->params['recruitmentUserInterview'];
        $body = str_replace('{fullname}', $modeluprofile->fullname, $body);
        $body = str_replace('{jabatan}', ($modelrecreq->jabatan) ? $modelrecreq->jabatan : '-', $body);
        $body = str_replace('{data}', str_replace(array("[", "]"), array(" "), json_encode($datancomp)), $body);
        if ($model->status == 2) {
