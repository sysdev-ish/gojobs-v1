...
class ChangecanceljoinController extends Controller
{
  ...
  public function actionCreate($id = null)
  {
    ...
    $approvalname = ArrayHelper::map(User::find()->where('role in(20,17)')->asArray()->all(), 'id', 'name');
    if ($id) {
      $model = $this->findModel($id);
      ...
      foreach ($namaquery as $key => $value) {
        if ($value->changereqdata) {
          ...
          // $checkdraft =  Changecanceljoin::find()->where(['userid'=>$value->userid, 'status'=>1, 'kategorydata'=>1])->count();
    ...
    if ($model->load(Yii::$app->request->post())) {
      ...
      if ($model->save()) {
        ...
        //klasifikasi 15 buat canceljoin, cek di malcounter
      }
